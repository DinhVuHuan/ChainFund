...
export const recordDonation = async (projectId, ethAmount, walletAddress, txHash) => {
  try {
    // Ensure campaign exists in DB (to satisfy FK). If missing, try to create from on-chain data.
    const { data: existingCamp, error: getCampErr } = await supabase
      .from('campaigns')
      .select('*')
      .eq('project_id', projectId)
      .single()

    if (!existingCamp && getCampErr && getCampErr.code !== 'PGRST116') {
      // unexpected error
      console.error('Error checking campaign existence:', getCampErr)
      throw getCampErr
    }

    if (!existingCamp) {
      try {
        // Prefer window.ethereum provider in browser; otherwise use configured RPC URL
        let provider
        if (typeof window !== 'undefined' && window.ethereum) {
          provider = new ethers.providers.Web3Provider(window.ethereum)
        } else if (CONTRACT_CONFIG && CONTRACT_CONFIG.rpcUrl) {
          provider = new ethers.providers.JsonRpcProvider(CONTRACT_CONFIG.rpcUrl)
        } else {
          throw new Error('No RPC provider available to fetch on-chain project')
        }

        const contract = new ethers.Contract(CONTRACT_CONFIG.address, GenesisABI.abi, provider)
        const proj = await contract.getProject(projectId)

        // map on-chain project to campaign fields
        const title = proj.title || `Project ${projectId}`
        const description = proj.description || ''
        const image_url = proj.imageURL || null
        const target_amount = parseFloat(ethers.utils.formatEther(proj.cost || ethers.constants.Zero))
        const raised_amount = parseFloat(ethers.utils.formatEther(proj.raised || ethers.constants.Zero)) || 0
        const owner_address = proj.owner || null
        const ts = proj.timestamp ? (proj.timestamp.toNumber ? proj.timestamp.toNumber() : Number(proj.timestamp)) : Math.floor(Date.now() / 1000)
        const expiresAt = proj.expiresAt ? (proj.expiresAt.toNumber ? proj.expiresAt.toNumber() : Number(proj.expiresAt)) : (ts + 864000)
        const duration_days = Math.max(1, Math.round((expiresAt - ts) / (24 * 60 * 60)))

        const { data: insertedCamp, error: insertCampErr } = await supabase
          .from('campaigns')
          .insert([
            {
              ...
              duration_days,
              image_url,
              owner_address: owner_address ? owner_address.toLowerCase() : null,
              status: 'OPEN',
        ...
        if (insertCampErr) {
          console.warn('Could not insert campaign fallback record:', insertCampErr)
        } else {
          console.log('Inserted campaign fallback record for project', projectId)
        }
      } catch (e) {
        console.warn('Failed to fetch on-chain project to create campaign record:', e)
      }
    }
    ...
    // Update raised_amount ở campaigns table (best-effort)
