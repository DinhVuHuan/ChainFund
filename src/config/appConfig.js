...
export const CONTRACT_CONFIG = {
  ...
  // 1) REACT_APP_CONTRACT_ADDRESS env var (recommended for deployments)
  // 2) src/abis/contractAddress.json (if generated by deploy script)
  // 3) fallback hardcoded address (useful for local dev)
  address: (() => {
    const envAddr = process.env.REACT_APP_CONTRACT_ADDRESS || '';
    if (envAddr) return envAddr;
    try {
      // require JSON file if present and contains `address` field
      // file may be empty during initial setup so wrap in try/catch
      // eslint-disable-next-line global-require
      const ca = require('../abis/contractAddress.json');
      if (ca && ca.address) return ca.address;
    } catch (e) {
      // ignore
    }
