  ...
  const loadCampaigns = async () => {
    try {
      ...
      if (campaigns && campaigns.length > 0) {
        ...
        // compute derived statuses; update DB only if a DB record exists
        const now = Date.now()
        for (const fc of formattedCampaigns) {
          ...
          // Only attempt to sync to DB when the original campaigns list contained a matching DB record
          // (we don't want to spam update calls when DB was wiped or missing)
  ...
  const refreshCampaigns = async () => {
    try {
      ...
      const mergedCampaigns = blockchainProjects.map((bp) => {
        const dbCampaign = dbCampaigns.find(dc => dc.project_id === bp.id) || null
        const raised = parseFloat(bp.raised)
        const cost = parseFloat(bp.cost)
        const expires = new Date(bp.expiresAt).getTime()

        // Normalize on-chain status to app-friendly values
        // bp.status comes from getAllProjectsFromBlockchain mapping: 'OPEN','APPROVED',...
        let onChainStatus = (bp.status || '').toString()
        let normalizedOnChainStatus = 'Active'
        if (/^APPROV/i.test(onChainStatus)) normalizedOnChainStatus = 'Approved'
        else if (/^OPEN/i.test(onChainStatus)) normalizedOnChainStatus = 'Active'
        else if (/^REVERT/i.test(onChainStatus)) normalizedOnChainStatus = 'Reverted'
        else if (/^DELET/i.test(onChainStatus)) normalizedOnChainStatus = 'Deleted'
        else if (/^PAID/i.test(onChainStatus)) normalizedOnChainStatus = 'PaidOut'

        // Derive status from on-chain numeric fields as fallback
        let derived = 'Active'
        if (expires <= now && raised < cost) derived = 'Expired'
        else if (raised >= cost && now <= expires) derived = 'Approved'

        // Prefer on-chain normalized status when available; fallback to derived
        const finalStatus = normalizedOnChainStatus || derived

        // Merge mapping (prefer on-chain for status and on-chain fields where authoritative)
        const merged = {
          id: bp.id,
          title: bp.title,
          owner: bp.owner,
        ...
        // If DB record exists and its status differs from on-chain, sync DB to on-chain normalized status
        if (dbCampaign && (dbCampaign.status || '') !== merged.status) {
